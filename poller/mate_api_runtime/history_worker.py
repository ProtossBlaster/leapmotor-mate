...
from api_v2_bridge import DB, NewAPIClient, connect_db, setting, set_setting
...
def sync_once():
    with connect_db() as db:
        ...
        username=crypto.decrypt(setting(db,'leapmotor_user'))
        password=crypto.decrypt(setting(db,'leapmotor_pass'))
        device=setting(db,'mate_device_id')
        if not device:
            device='mate-'+secrets.token_hex(16)
            set_setting(db,'mate_device_id',device)
        vins=[row[0] for row in db.execute('SELECT vin FROM vehicles')]
    if not username or not password or not vins:return
