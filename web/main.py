...
# Same background timer for per-trip elevation gain/loss (Open-Elevation lookup on the GPS track).
...
def _ctx(**kwargs):
    ...
    # Same piggyback for per-trip elevation gain/loss (Open-Elevation) — no-op unless due.
...
async def trip_calc_elevation(request: Request, trip_id: int):
    """'Calculate elevation' button: on-demand Open-Elevation lookup for this trip (and its merged
    segments), for trips predating the feature or ones the background sweep hasn't reached/solved
    yet. On success reload the page so the dislivello figures appear; on a genuine miss show an
    inline message and change nothing."""
