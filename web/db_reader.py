...
def _telemetry_frozen(prev: dict, cur: dict) -> bool:
    """True when `cur` repeats `prev`'s speed, SoC AND position while claiming a real driving
    speed — physically impossible for a car that's actually moving, and the substance-level twin
    of the write-time stale-frame guard (poller/recorder.py #128): that guard catches an identical
    RAW cloud timestamp, but if the cloud re-serves a cached snapshot wrapped in a fresh timestamp
    (each poll still advances), the payload underneath — speed, SoC, GPS — stays frozen while the
    timestamp doesn't, and #128's identity check misses it entirely."""
...
def _filter_frozen_telemetry(positions: list[dict]) -> list[dict]:
    """Drop a run of trip_positions samples where the cloud kept re-serving a CACHED vehicle
    snapshot — speed/SoC/GPS frozen — while its own wrapper timestamp still advanced each poll
    (see _telemetry_frozen). Every sample here is DRIVING by construction (trip_positions only
    records driving polls), so a run this way above walking pace is never legitimate — unlike
    #128's stale-frame guard it runs AFTER the fact, so it also cleans up trips already recorded
    with a live cloud hiccup. Keeps the point right before the freeze as the last-known-good
    anchor; the point where real telemetry resumes becomes the anchor after it — the resulting
    recorded_at gap is exactly what the trip-profile chart already renders as a break (same logic
    as a genuine signal-loss gap). A run shorter than _FROZEN_MIN_RUN_S is left alone: too brief to
    trust over the (small) risk of discarding real data."""
    n = len(positions)
    if n < 3:
        return positions
    dup = [False] * n
...
def _interpolate_elevation(positions: list[dict]) -> list[dict]:
    ...
    genuine jumps) — the same technique route-elevation profiles use to draw a smooth line from
    sparse samples. A leading/trailing gap (no known value on ONE side) is left None — never
    extrapolated beyond what was actually measured. Mutates and returns `positions`."""
...
def _trip_group_stats(parent: dict, children: list) -> dict:
    ...
    # Unlike regen_kwh, None here means "not enriched yet" (not "zero") — summing None-as-0 across
    # a merged group would show a misleading "+0 m" while segments are still waiting on the
    # Open-Elevation sweep. Only aggregate once every segment has a value.
...
def get_trip_detail(trip_id: int) -> Optional[dict]:
    ...
    # `positions` — so the trip-profile chart, the map track and the speed stats below all agree on
    # what actually happened, instead of the chart alone showing a gap while stats stay skewed.
    positions = _filter_frozen_telemetry([dict(p) for p in positions])
    # Whether the chart has anything to draw a Quota line from — checked BEFORE interpolation
    # (which needs a real point to fill from). A trip enriched before per-point storage existed
    # has elevation_gain_m/loss_m set but every trip_positions.elevation_m still NULL: the "already
    # enriched" gain/loss must not hide the recalculate button in that case (see template).
    elevation_profile_available = any(p.get("elevation_m") is not None for p in positions)
    # elevation_m is only ever fetched for the DOWNSAMPLED subset the enrichment sweep queried
    # Open-Elevation for (see get_trip_points_for_elevation) — fill the rest by interpolation so
    # the chart draws a smooth line instead of one with a break at every un-sampled point.
...
def get_trips_needing_elevation(limit: int = 4) -> list[dict]:
    """Finalized trips (any segment, not just top-level — elevation is per-segment like regen_kwh,
    see _trip_group_stats) that aren't elevation-enriched yet, under the retry ceiling, with a real
    GPS track and non-trivial distance (skip parking-lot-only trips)."""
    db = _get()
    rows = db.execute(
...
def get_trip_points_for_elevation(trip_id: int, max_points: int = 60) -> list[dict]:
    """Lat/lon track of THIS trip segment only (not a merged group — enrichment runs per segment),
    downsampled to at most ``max_points`` to keep the Open-Elevation batch small and less noisy.
    Frozen-telemetry duplicates (see _filter_frozen_telemetry) are dropped BEFORE downsampling —
    left in, a long freeze wastes several of the ``max_points`` slots on the same repeated
    coordinate (index-based downsampling has no notion of "already sampled this spot"), coarsening
    the profile over the rest of the trip for no benefit: identical points contribute zero
    elevation delta anyway."""
    db = _get()
    rows = db.execute(
...
def store_point_elevations(elevations_by_id: dict) -> None:
    """Persist the per-point altitude (metres) the enrichment sweep just fetched, keyed by
    trip_positions.id — this is the sparse (downsampled) subset the chart's _interpolate_elevation
    fills the gaps between. `{}`/None is a no-op (nothing usable came back)."""
...
def store_trip_elevation(trip_id: int, gain, loss) -> None:
    """Record an elevation-enrichment attempt. Always bumps elev_tried; with a result (gain/loss not
    None) also stores it and marks elev_done=1 so the sweep stops re-fetching it."""
    db = _conn_rw()
    if gain is None or loss is None:
        db.execute("UPDATE trips SET elev_tried = COALESCE(elev_tried, 0) + 1 WHERE id=?", (trip_id,))
    else:
        db.execute(
            """UPDATE trips SET elev_tried = COALESCE(elev_tried, 0) + 1,
                   elevation_gain_m=?, elevation_loss_m=?, elev_done=1 WHERE id=?""",
            (gain, loss, trip_id))
    db.commit()
