...
elevation_enrich only queries Open-Meteo for a DOWNSAMPLED subset of a trip's GPS points (see
...
def test_get_trip_detail_interpolates_the_sparse_track(tmp_path, monkeypatch):
    ...
    # Open-Meteo value — mirrors what elevation_enrich.py actually persists.
...
def test_elevation_profile_unavailable_when_trip_predates_per_point_storage(tmp_path, monkeypatch):
    """Regression: a trip enriched BEFORE per-point storage existed has elevation_gain_m/loss_m set
    but every trip_positions.elevation_m still NULL — the aggregate being present must not hide this
    from the chart (or the 'Calculate elevation' button, see trip_detail.html)."""
    pdb = D.Database(str(tmp_path / "t.db"))
    monkeypatch.setattr(DR, "DB_PATH", str(tmp_path / "t.db"))
    now = datetime.now(timezone.utc)
    started, ended = now - timedelta(minutes=10), now - timedelta(minutes=5)
