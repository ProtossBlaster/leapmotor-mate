...
poller/recorder.py's stale-frame guard (#128) catches the cloud re-serving a byte-identical frame —
same raw timestamp. But if the cloud instead re-serves a CACHED snapshot wrapped in a fresh timestamp
each poll (the payload underneath — speed/SoC/GPS — frozen, only the wrapper timestamp moving), that
guard never fires: every poll looks like a new frame. The symptom users see is exactly this: a flat
speed plateau for several real minutes, then an abrupt SoC/speed snap back once the cloud catches up.
