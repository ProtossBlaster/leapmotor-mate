"""Per-trip elevation gain/loss (dislivello) + outside temperature, from the GPS track looked up
against Open-Meteo. No network in tests — `fetch_elevations` / `fetch_trip_temperature` are
monkeypatched to scripted values.
"""
...
# ── temperature helpers: pure functions ─────────────────────────────────────────────────────

def test_temp_at_hour_floors_to_the_hour():
    times = ["2026-07-19T09:00", "2026-07-19T10:00", "2026-07-19T11:00"]
    temps = [18.0, 20.0, 22.0]
    assert EE._temp_at_hour(times, temps, datetime(2026, 7, 19, 10, 15, tzinfo=timezone.utc)) == 20.0
    assert EE._temp_at_hour(times, temps, datetime(2026, 7, 19, 11, 50, tzinfo=timezone.utc)) == 22.0


def test_temp_at_hour_absent_returns_none():
    times = ["2026-07-19T10:00"]
    temps = [20.0]
    assert EE._temp_at_hour(times, temps, datetime(2026, 7, 20, 10, 0, tzinfo=timezone.utc)) is None
    assert EE._temp_at_hour(times, temps, None) is None


def test_temp_at_hour_skips_null():
    times = ["2026-07-19T10:00", "2026-07-19T11:00"]
    temps = [None, 22.0]
    assert EE._temp_at_hour(times, temps, datetime(2026, 7, 19, 10, 30, tzinfo=timezone.utc)) is None
    assert EE._temp_at_hour(times, temps, datetime(2026, 7, 19, 11, 30, tzinfo=timezone.utc)) == 22.0


# ── sweep: DB-backed, fetch_elevations + fetch_trip_temperature stubbed ──────────────────────

def _setup(tmp_path, monkeypatch, n_points=5):
    """One finalized trip with a straight-line GPS track, feature enabled. Temperature is stubbed to
    None by default (no network); tests that care re-stub it."""
    ...
    monkeypatch.setattr(EE, "fetch_trip_temperature", lambda *a, **k: (None, None))
    return pdb
...
def test_sweep_stores_start_and_end_temperature(tmp_path, monkeypatch):
    pdb = _setup(tmp_path, monkeypatch)
    monkeypatch.setattr(EE, "fetch_elevations", lambda pts: [100, 110, 120, 115, 105])
    monkeypatch.setattr(EE, "fetch_trip_temperature", lambda *a, **k: (21.5, 19.0))
    EE._sweep_now()
    s, e = pdb._conn.execute(
        "SELECT outside_temp_start_c, outside_temp_end_c FROM trips WHERE id=1").fetchone()
    assert (s, e) == (21.5, 19.0)


def test_temperature_kept_even_when_elevation_misses(tmp_path, monkeypatch):
    """Elevation and temperature are independent in the same pass: temperatures that came back are
    stored even if the elevation lookup missed, and the trip stays not-done so elevation retries."""
    pdb = _setup(tmp_path, monkeypatch)
    monkeypatch.setattr(EE, "fetch_elevations", lambda pts: None)
    monkeypatch.setattr(EE, "fetch_trip_temperature", lambda *a, **k: (12.0, 10.0))
    EE._sweep_now()
    s, e, done, tried = pdb._conn.execute(
        "SELECT outside_temp_start_c, outside_temp_end_c, elev_done, elev_tried FROM trips WHERE id=1"
    ).fetchone()
    assert (s, e) == (12.0, 10.0)   # temperatures kept
    assert done == 0                # elevation not marked done (missed) → re-selected next sweep
    assert tried == 1               # attempt counted
...
def test_sweep_gives_up_after_retry_ceiling(tmp_path, monkeypatch):
    ...
    longer selected, so a 4th sweep leaves elev_tried at 3 (enrichment abandoned, shows '—')."""
    pdb = _setup(tmp_path, monkeypatch)
    monkeypatch.setattr(EE, "fetch_elevations", lambda pts: None)
    for _ in range(4):
        EE._sweep_now()
...
def test_sweep_skips_trip_with_single_point(tmp_path, monkeypatch):
    ...
    EE._sweep_now()
    assert calls == []  # never even called Open-Meteo
    gain, loss, tried, done = _row(pdb)
...
def test_recalc_trip_ignores_the_retry_ceiling(tmp_path, monkeypatch):
    """A trip the sweep already gave up on (elev_tried past the ceiling) is invisible to
    get_trips_needing_elevation, but the manual button must still recover it."""
    pdb = _setup(tmp_path, monkeypatch)
...
def test_recalc_trip_recovers_pre_feature_trip(tmp_path, monkeypatch):
    """A trip recorded before this feature existed has elev_tried/elev_done at their column defaults
    (0) — recalc_trip must enrich it exactly like a fresh trip."""
    pdb = _setup(tmp_path, monkeypatch)
...
def test_recalc_trip_covers_every_merged_segment(tmp_path, monkeypatch):
    """Elevation is per-segment (like regen_kwh) — recalculating a merged group's parent must also
    enrich its children, not just the parent row."""
